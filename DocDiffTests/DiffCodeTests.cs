...
public class DiffCodeTests
{
    ...
    [Test]
    public void create_a_text_diff_code_from_complex_changes_and_reapply()
    {
        var code = DiffCode.BuildDiffCode(new Differences(Samples.ComplexLeft, Samples.ComplexRight, Differences.PerCharacter).ToList());

        Console.WriteLine($"Encoded to {code.Length} chars");
        Console.WriteLine(code);

        var right = DiffCode.BuildRevision(Samples.ComplexLeft, code);
        
        Console.WriteLine(right);
        
        Assert.That(right, Is.EqualTo(Samples.ComplexRight));
    }
    ...
    [Test]
    public void rebuild_difference_fragments_from_a_large_text_binary_diff_code()
    {
        var diff = new Differences(Samples.LongLeft, Samples.LongRight, Differences.PerWord);
        var originalFrags = diff.ToList();
        ...
        var code = DiffCode.StorageDiffCode(diff);
